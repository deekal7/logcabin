...
def cpu_slow(slow_server_config, slow_ip, slow_pids):
    quota=50000
    period=1000000
    print("Starting CPU slow server")
    print(slow_pids)
    ssh -i ~/.ssh/id_rsa @(slow_ip) "sudo -S sh -c 'sudo -S mkdir /sys/fs/cgroup/cpu/db'"
    ssh -i ~/.ssh/id_rsa @(slow_ip) @("sudo -S sh -c 'sudo -S echo {} > /sys/fs/cgroup/cpu/db/cpu.cfs_quota_us'".format(quota))
    ssh -i ~/.ssh/id_rsa @(slow_ip) @("sudo -S sh -c 'sudo -S echo {} > /sys/fs/cgroup/cpu/db/cpu.cfs_period_us'".format(period))
    
    for slow_pid in slow_pids.split():
        ssh -i ~/.ssh/id_rsa @(slow_ip) @("sudo -S sh -c 'sudo -S echo {} > /sys/fs/cgroup/cpu/db/cgroup.procs'".format(slow_pid))
...
def memory_contention(slow_server_config, slow_ip, slow_pids):
    print("Injecting memory contention")
    ssh -i ~/.ssh/id_rsa @(slow_ip) "sudo -S sh -c 'sudo -S mkdir /sys/fs/cgroup/memory/db'"
    ...
    ssh -i ~/.ssh/id_rsa @(slow_ip) @("sudo -S sh -c 'sudo -S echo {} > /sys/fs/cgroup/memory/db/memory.limit_in_bytes'".format(5 * 1024 * 1024))   # 5MB
    
    for slow_pid in slow_pids.split():
        ssh -i ~/.ssh/id_rsa @(slow_ip) @("sudo -S sh -c 'sudo -S echo {} > /sys/fs/cgroup/memory/db/cgroup.procs'".format(slow_pid))
    print("Injected memory contention")

def kill_process(ip, pids):
    print("Killing ", pids)
    for pid in pids.split():
        ssh -i ~/.ssh/id_rsa @(ip) f"sudo -S sh -c 'kill -9 {pid}'"
